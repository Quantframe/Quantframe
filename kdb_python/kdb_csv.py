...
def update(config,update_kdb,update_csv,last_date_record,name,new_data,reset_index=True,set_index=False,index_name='trade_date'):
    ...
    if update_kdb:
        ...
        if data_kdb_save.shape[0]>0:
            if (config['runD']): print(dt.datetime.now(), '开始更新KDB数据：,过去文件最新日期: ' + str(last_date_record['KDB'][i]) + '; 更新开始日期: ' + str(data_kdb_save.index.min()))
            ...
            query = name+': aj[`ts_code`'+index_name+';'+name+';table_new]'
    ...
    if update_csv:
        ...
        if data_csv_save.shape[0] > 0:
            if (config['runD']): print(dt.datetime.now(), '开始更新CSV数据：,过去文件最新日期: ' + str(last_date_record['CSV'][i]) + '; 更新开始日期: ' + str(data_csv_save.index.min()))
            data = pd.read_csv(config['place_load']['csv']+name+'.csv')
            data = data.set_index(index_name)
